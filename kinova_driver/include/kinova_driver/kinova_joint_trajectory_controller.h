...
{
public:
  JointTrajectoryController(kinova::KinovaComm &kinova_comm,
                            ros::NodeHandle &n);
...
private:
  ...
  sensor_msgs::JointState current_joint_state_;  // Note: Not used anymore?
  ...
  // reflash the robot state and publish the joint state: either by timer or
  // thread
  ...
  // WIP: More advanced controlling
  std::map<int, std::vector<double>> joint_id_to_goal_distances_;

  std::vector<double> additional_time_needed_;
