...
void make1DofMarker(const std::string& frame_id, const std::string& axis,
    unsigned int interaction_mode, const tf::Vector3& position, const std::string& description,
...
void sendArmPoseGoal(geometry_msgs::PoseStamped& endeffector_pose)
{
    ...
    ROS_INFO_STREAM("Goal to arm pose actionlib: \n"
        ...
        << ", z: " << goal.pose.pose.position.z << ", qx: " << goal.pose.pose.orientation.x
        << ", qy: " << goal.pose.pose.orientation.y << ", qz: " << goal.pose.pose.orientation.z
...
void sendArmJointGoal(const std::string marker_name, double joint_offset)
{
    ...
    ROS_INFO(" joint goal is set as : %f, %f, %f, %f, %f, %f (degree)\n", goal.angles.joint1,
        goal.angles.joint2, goal.angles.joint3, goal.angles.joint4, goal.angles.joint5,
        goal.angles.joint6);
    ...
    case visualization_msgs::InteractiveMarkerFeedback::MOUSE_DOWN:
        ...
        if (mouse_was_up = true)
        {
            ...
            else
            {
                ...
                //                ROS_INFO_STREAM( s.str() << ": mouse DOWN (refers to current
                //                status): "
...
void currentPoseFeedback(const kinova_msgs::KinovaPoseConstPtr pose_command)
{
    ...
    if (getCurrentCommand == false)
    {
        ...
        pose.setOrigin(
            tf::Vector3(current_pose_command.X, current_pose_command.Y, current_pose_command.Z));
        tf::Quaternion q = kinova::EulerXYZ2Quaternion(
            current_pose_command.ThetaX, current_pose_command.ThetaY, current_pose_command.ThetaZ);
        pose.setRotation(q);
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber armJoint_sub
        = nh.subscribe("/" + tf_prefix_ + "_driver/out/joint_command", 1, &currentJointsFeedback);
    ros::Subscriber armCartesian_sub
        = nh.subscribe("/" + tf_prefix_ + "_driver/out/cartesian_command", 1, &currentPoseFeedback);

    armJoint_interMark_server.reset(new interactive_markers::InteractiveMarkerServer(
        tf_prefix_ + "_interactive_control_Joint", "", false));
    armPose_interMark_server.reset(new interactive_markers::InteractiveMarkerServer(
    ...
    make1DofMarker(tf_prefix_ + "_link_1", "z",
        visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "1st Axis",
        "marker_joint1");
    make1DofMarker(tf_prefix_ + "_link_2", "z",
        visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "2nd Axis",
        "marker_joint2");
    make1DofMarker(tf_prefix_ + "_link_3", "z",
        visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "3rd Axis",
        "marker_joint3");
    make1DofMarker(tf_prefix_ + "_link_4", "z",
        visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "4th Axis",
        "marker_joint4");
    if (arm_joint_number_ == 6)
    {
        make1DofMarker(tf_prefix_ + "_link_5", "z",
            visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "5th Axis",
            "marker_joint5");
        make1DofMarker(tf_prefix_ + "_link_6", "z",
            visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS, position, "6th Axis",
            "marker_joint6");
    }
