...
KinovaKinematics::KinovaKinematics(
    const ros::NodeHandle& node_handle, std::string& kinova_robotType)
    : kinova_robotType_(kinova_robotType)
{
    ...
    if (kinova_robotType_.substr(0, 4) == "j2n4" || kinova_robotType_.substr(0, 4) == "m1n4")
    {
        ...
        double aa = wrist_deg_ / 2 * M_PI / 180.0, sa = sin(aa),
               s2a = sin(2 * aa); // temp parameters
    ...
    else if (kinova_robotType_.substr(0, 4) == "j2n6" || kinova_robotType_.substr(0, 4) == "m1n6")
    {
        ...
        double DH_d[6] = { D1_, 0, -e2_, -(D3_ + sa / s2a * D4_),
            -(sa / s2a * D4_ + sa / s2a * D5_), -(sa / s2a * D5_ + D6_) };
...
tf::Transform KinovaKinematics::DHParam2Transform(float d, float theta, float a, float alpha)
{
    ...
    rot_matrix.setValue(cos(theta), -sin(theta) * cos(alpha), sin(theta) * sin(alpha), sin(theta),
        cos(theta) * cos(alpha), -cos(theta) * sin(alpha), 0, sin(alpha), cos(alpha));
...
/*                                    */
/**************************************/
// IMPORTANT!!! In the robot DSP chip, the classical D-H parameters are used to define the robot.
// Therefore, the frame definition is different comparing with the frames in URDF model.
void KinovaKinematics::updateForward(float* Q)
{
    ...
    broadcaster_.sendTransform(tf::StampedTransform(
        transform, ros::Time::now(), baseFrame, concatTfName(tf_prefix_, "link_base")));
    double DH_theta_i;
    for (int i = 0; i < arm_joint_number_; i++)
    {
        ...
        if (i == 0)
        {
            broadcaster_.sendTransform(tf::StampedTransform(transform, ros::Time::now(),
                concatTfName(tf_prefix_, "link_base"), concatTfName(tf_prefix_, "link_1")));
        }
        else
        {
            broadcaster_.sendTransform(tf::StampedTransform(transform, ros::Time::now(),
                concatTfName(tf_prefix_, "link_", i), concatTfName(tf_prefix_, "link_", i + 1)));
        }
    }
}
